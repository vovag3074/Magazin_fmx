...
uses
  ...
  FMX.TMSFNCBitmapContainer, FMX.TMSFNCPopup, FMX.Objects;
...
type
  TfmSelAgn = class(TForm)
    ...
    procedure tlSityFocusedNodeChanged(Sender: TObject; ANode:
      TTMSFNCTreeViewVirtualNode);
    procedure tlAgnGetNodeTextColor(Sender: TObject; ANode:
      TTMSFNCTreeViewVirtualNode; AColumn: Integer; var ATextColor: TTMSFNCGraphicsColor);
    procedure tlAgnGetNodeSelectedTextColor(Sender: TObject; ANode:
      TTMSFNCTreeViewVirtualNode; AColumn: Integer; var ATextColor: TTMSFNCGraphicsColor);
    procedure tlAgnGetNodeSelectedColor(Sender: TObject; ANode:
...
procedure TfmSelAgn.btFTSMouseEnter(Sender: TObject);
var
  p, r: TRectF;
  s: string;
begin
  ...
  begin
    ...
    HintPanel.CalloutPosition := TCalloutPosition.Top;
    HintPanel.Position.X := p.Left + TControl(Sender).Width / 2 - HintPanel.Width / 2;
    HintPanel.Position.Y := p.Bottom + 10;
...
procedure TfmSelAgn.btFTSMouseLeave(Sender: TObject);
begin
...
procedure TfmSelAgn.eFindKeyDown(Sender: TObject; var Key: Word; var KeyChar:
...
const
  myWhere =
    ' join FTS$POKUPATEL$WRD FTS$POKUPATEL$WRD$$ on FTS$POKUPATEL$WRD$$.FTS$WRD starting ''%%'' and ' +
    'FTS$POKUPATEL$WRD$$.FTS$NO_AGN = AGENTS.NO_AGN ';
  myQuery = 'select first (10) skip (:PG) NO_AGN, ' + 'NO_SITY, ' + 'AG_NAME, '
    + 'AG_DOLG, ' + 'AG_PRED, ' + 'FULL_NAME as Upper_full_name, ' + 'STATUS, '
    + 'ST_NAME, ' + 'Sum_Skidka, ' + 'IS_SKIDKA, ' +
    '(AG_NAME || '' '' || ST_NAME) as FULL_NAME ' + 'from SITY_TABLE, AGENTS ';
  EndQuery_1 = 'where AGENTS.IS_DEL = 0 and ' + 'AGENTS.NO_SITY = SITY_TABLE.NO_ST ';
  EndQuery_2 = 'where AGENTS.IS_DEL = 0 and AGENTS.NO_SITY = :NS and' +
    'AGENTS.NO_SITY = SITY_TABLE.NO_ST ';
var
  S: string;
  SL: tStringList;
  I: Integer;
...
const
  myWhere = ' FROM AGENTS JOIN FTS$POKUPATEL$WRD FTS$POKUPATEL$WRD0 ' +
    ' ON FTS$POKUPATEL$WRD0.FTS$SYN STARTING WITH ''%%'' ' +
    ' AND FTS$POKUPATEL$WRD0.FTS$NO_AGN = AGENTS.NO_AGN ' +
    ' WHERE AGENTS.IS_DEL = 0 ' + ' PLAN JOIN ( FTS$POKUPATEL$WRD0 INDEX ( FTS$POKUPATEL$SYN ), '
    + ' AGENTS INDEX ( PK_AGENTS ) ) ';
  myQuery = ' SELECT first (10) skip(:PG) AGENTS.NO_AGN, AGENTS.FULL_NAME_STD, '
    + ' AGENTS.AG_DOLG, AGENTS.AG_PRED, AGENTS.AG_BANK_PRED ' +
    ' , AGENTS.SUM_SKIDKA, AGENTS.IS_SKIDKA ';
var
  S: string;
  I: Integer;
  Node: TTMSFNCTreeViewNode;
 ...
 // ShowInfoEx('1');
  ...
  begin
    ...
    repeat
      ...
      Node.Text[1] := qFTS.FieldByName('AG_DOLG').AsFloat.ToString;
      Node.Text[2] := FloatToStr(qFTS.FieldByName('AG_PRED').AsFloat + qFTS.FieldByName
        ('AG_BANK_PRED').asFloat);
      if qFTS.FieldByName('IS_SKIDKA').AsInteger = 1 then
      begin
        Node.Values[0].CollapsedIconName := 'Item5';
        Node.Values[0].ExpandedIconName := 'Item5';
...
procedure TfmSelAgn.tlAgnGetNodeSelectedColor(Sender: TObject; ANode:
  TTMSFNCTreeViewVirtualNode; var AColor: TTMSFNCGraphicsColor);
begin
...
procedure TfmSelAgn.tlAgnGetNodeSelectedTextColor(Sender: TObject; ANode:
  TTMSFNCTreeViewVirtualNode; AColumn: Integer; var ATextColor: TTMSFNCGraphicsColor);
begin
  if ANode.Node.Text[2].ToDouble > 0 then
  begin
...
procedure TfmSelAgn.tlAgnGetNodeTextColor(Sender: TObject; ANode:
  TTMSFNCTreeViewVirtualNode; AColumn: Integer; var ATextColor: TTMSFNCGraphicsColor);
begin
  if ANode.Node.Text[2].ToDouble > 0 then
  begin
...
procedure TfmSelAgn.tlSityFocusedNodeChanged(Sender: TObject; ANode:
  TTMSFNCTreeViewVirtualNode);
begin
  eFind.Text := '';
  ListAgent;
